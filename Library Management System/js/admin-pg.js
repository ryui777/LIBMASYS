...
function renderBooks() {
  ...
  list.innerHTML = '';
  if (!books.length) {
    list.innerHTML = '<div class="empty-state"><div class="empty-icon">📚</div><p>No books yet. Add one!</p></div>';
    return;
  }
  books.forEach(book => {
...
window.addOrUpdateBook = async function() {
  ...
  try {
    let res;
    if (editingId) {
      res = await fetch('api/books.php', { method:'PUT', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ id: editingId, title, author, category, img }) }).then(r => r.json());
    } else {
      res = await fetch('api/books.php', { method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ title, author, category, img }) }).then(r => r.json());
    }
    ...
    if (editingId) {
      books = books.map(b => b.id === editingId ? { ...b, title, author, category, img } : b);
    } else {
      books.push({ id: Date.now(), title, author, category, img });
    }
...
window.deleteBook = async function(id) {
  ...
  try {
    const res = await fetch('api/books.php', { method:'DELETE', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ id }) }).then(r => r.json());
    if (res.success) { showToast('Book deleted.', 'info'); loadBooks(); }
    else showToast(res.error || 'Failed.', 'error');
  } catch {
    books = books.filter(b => b.id != id);
    localStorage.setItem('books', JSON.stringify(books));
    showToast('Book deleted.', 'info'); renderBooks();
  }
};

function clearForm() {
  ['title','author','category','image'].forEach(id => document.getElementById(id).value = '');
}
